...
flowchart TD
    A[RD 開發完成]
    B[CI-1: Pre-Commit Gate Check<br/>部分回歸測試（重要測項）<br/>快速驗證（約 15-30 分鐘）]
    C[獲得提交資格<br/>72 小時有效期限]
    D[CI-MTBF: 壓力測試<br/>Mean Time Between Failures<br/>每天 1-2 班次<br/>長時間穩定性驗證]
    E[獲得 Merge Ticket<br/>72 小時有效期限]
    F[RD 決定提交時機]
    G[版控變更偵測<br/>檢查程式碼是否被修改]
    H[Code Merge<br/>合併到 main/develop]
    I[CI-0: Internal Verification<br/>每日 10 輪全回歸測試<br/>02:00 AM 執行<br/>100% 測試覆蓋率]
    J[Deploy to Staging<br/>部署到測試環境]
    K[UAT Verification<br/>PM/Stakeholder 驗收]
    L[Deploy to Prod<br/>部署到正式環境]
...
### 觸發時機
- **必須通過 MTBF**：CI-1 通過後，所有提交必須完成 MTBF 壓力測試，且通過後方可進入後續流程
